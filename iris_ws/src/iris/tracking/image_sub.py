...
class DetectObj(Node):
    ...
    def __init__(self):
        ...
        self.image_sub = self.create_subscription(Image,"tracking/image_in",self.callback,rclpy.qos.QoSPresetProfiles.SENSOR_DATA.value)
        self.image_out_pub = self.create_publisher(Image, "tracking/image_out", 1)
        self.image_tuning_pub = self.create_publisher(Image, "tracking/image_tuning", 1)
        self.ball_pub  = self.create_publisher(Point,"tracking/detected_object",1)
