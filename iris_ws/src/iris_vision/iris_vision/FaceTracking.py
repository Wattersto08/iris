...
class ImageSubscriber(Node):
    ...
    def listener_callback(self, inputdata: Image):
        ...
        current_frame = self.br.imgmsg_to_cv2(inputdata, desired_encoding='mono8') #color = bgr8  grey = mono8
        
        faces = trained_face_data.detectMultiScale(current_frame, 1.1, 4)
        ...
        if len(faces) == 1:
          for (x, y, w, h) in faces:
            ...
            errorX_scaled = int((errorX/240)*400)
            errorY_scaled = int((errorY/320)*400)

            msg = Int32()
            msg.data = m.floor(errorY_scaled)
            self.publisherY.publish(msg)
            msg.data = m.floor(errorX_scaled)
            self.publisherX.publish(msg)

        else:
            self.get_logger().info('No Face Detected!')
            
        image_message = self.br.cv2_to_imgmsg(current_frame)        
